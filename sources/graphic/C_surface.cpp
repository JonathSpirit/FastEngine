        ...
        g_surface(nullptr)
{
    ...
    {
        this->g_surface = SDL_CreateRGBSurfaceWithFormatFrom(r.g_surface->pixels,
                                                             r.g_surface->w,
                                                             r.g_surface->h, 32,
...
bool Surface::blitSurface(const Surface& src, const std::optional<SDL_Rect>& srcRect, std::optional<SDL_Rect>& dstRect)
{
    ...
    return SDL_BlitSurface(src.get(),
                           srcRect.has_value() ? &srcRect.value() : nullptr,
                           this->g_surface,
...
bool Surface::addBorder(int borderSize, const fge::Color& color)
{
    ...
    SDL_Surface* transparentBorderSurface = SDL_CreateRGBSurfaceWithFormat(0, this->g_surface->w+borderSize*2,
                                                                           this->g_surface->h+borderSize*2, 32,
                                                                           this->g_surface->format->format);
    if (transparentBorderSurface == nullptr)
    {
        return false;
    }

    SDL_FillRect(transparentBorderSurface, nullptr, SDL_MapRGBA(transparentBorderSurface->format, color._r, color._g, color._b, color._a));
